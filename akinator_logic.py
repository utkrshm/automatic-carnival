...
class Akinator:
    ...
    def _calc_entropy(self, probs: List[float]) -> float:
        if not probs or sum(probs) < 1e-9:
            return 0.0
        s = sum(probs)
        if abs(s - 1.0) > 1e-6 and len(probs) > 1:
            probs = [p / s for p in probs]
        return -sum(p * math.log2(p) for p in probs if p > 1e-9)

    def _get_top_candidates(self, n: Optional[int] = None) -> List[Tuple[str, float]]:
    ...
    def _calc_info_gain_subset(self, subset_candidates: List[str], unasked_attrs: List[str]) -> Optional[str]:
        ...
        for attr in unasked_attrs:
            ...
            entropy_yes = self._calc_entropy([p / sum_yes for p in probs_yes if sum_yes > 1e-9])
            entropy_no = self._calc_entropy([p / sum_no for p in probs_no if sum_no > 1e-9])
    ...
    def _get_current_guess(self) -> Tuple[Optional[str], float]:
        ...
        if not active_probs:
            return None, 0.0
        sorted_probs = sorted(active_probs.items(), key=lambda x: x[1], reverse=True)
    ...
    def _update_probs(self, attr: str, answer: float) -> bool:
        ...
        if current_sum < 1e-9:
            self.probabilities = {name: 0.0 for name in self.people}
            return False
    ...
    def select_next_question(self) -> Optional[str]:
        ...
        if not active_names:
            return None
        next_attr = None
        if self.n_questions_asked >= self.MIN_QUESTIONS // 2:
            top_candidates = self._get_top_candidates(self.TOP_N_CANDIDATES)
            top_n_names = [name for name, prob in top_candidates if prob > 1e-9]
    ...
    def process_answer(self, attribute_key: str, answer_numeric: float) -> Dict[str, Any]:
        ...
        if not self._update_probs(attribute_key, answer_numeric):
            return {"status": "finished", "message": "No candidates remain. Contradiction or not in database.", "guess": None, "certainty": 0.0, "top_candidates": self._get_top_candidates(5)}
    ...
    def process_mistaken_guess(self, wrong_guess_name: str) -> Dict[str, Any]:
        if wrong_guess_name in self.probabilities:
            ...
            if current_sum > 1e-9:
                ...
                return {"status": "finished", "message": "No candidates remain after mistaken guess.", "guess": None, "certainty": 0.0, "top_candidates": self._get_top_candidates(5)}
        self.RETRY = True
        next_attribute = self.select_next_question()
        if next_attribute:
            return {
